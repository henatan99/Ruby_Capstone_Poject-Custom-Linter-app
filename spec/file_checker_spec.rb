...
describe CheckFile do 
    ...
    describe "lines" do 
        ...
        it "with index 1 returns an array of starting column of each lines" do 
            expect(test.lines[1][1]).to eql(4)
        end

        it "with index 1 returns an array of starting column of each lines" do 
            expect(test.lines[1]).to eql([0, 4, 8, 4, 0, 4, 13, 4, 0, 4, 4, 8, 4, 0, 4, 8, 8, 8, 12, 8, 8, 4, 0, 0])
        end
        ...
        it "with index 0 returns an array of unnessasry space columns of each lines" do 
            expect(test.lines[0][2]).to eql([21, 22])
        end
        it "with index 0 returns an array of unnessasry space columns of each lines" do 
    ...
    describe "line_level" do 
        ...
        it "with index 1 returns an array of line number of def starts" do 
            expect(test.line_level[0]).to eql([0, 1, 2, 1, 1, 1, 2, 1, 1, 1, 2, 2, 1, 1, 1, 2, 2, 2, 3, 2, 2, 1, 0, 0])
        end
    end
    ...
    describe "check_indent" do 
        ...
        it "returns an array of pairs" do 
            expect(test.check_indent[0]).to eql([7, 5])
        end
        it "returns an array of pairs" do 
            expect(test.check_indent).to eql([[7, 5], [11, -4]])
        end
    end
